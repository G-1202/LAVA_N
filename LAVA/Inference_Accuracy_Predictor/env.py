...
class Environment:
    ...
    def get_video_chunk(self, knob, s, r):

        self.config.append([knob, s, r])
        index = self.video_chunk_counter - 1
        index += self.c_start
        video_chunk_size = self.df.loc[(index, knob, s, r), 'Size']
        self.bw_use.append(self.df.loc[(index, knob, s, r), 'Bitrate'] / 1000)

        encode_t = ENCODE_T[knob*16+s*4+r]
        ...
        f = self.df.loc[(index, knob, s, r), 'Accuracy']
        small = self.df.loc[(index, knob, s, r), 'Small']
        mid = self.df.loc[(index, knob, s, r), 'Mid']
        large = self.df.loc[(index, knob, s, r), 'Large']
        move = self.df.loc[(index, knob, s, r), 'Move']
        self.F1.append(f)
        ...
        self.skip.append(s)
        self.frame.append((index+1, knob, s, r))
        ...
        if BW_estimate > len(self.bw):
            bw_est = bw_3(self.bw, self.duration, self.end_t, self.start)
        else:
            bw_est = bw_4(self.bw[-BW_estimate:], self.duration[-BW_estimate:], self.end_t[-BW_estimate:], self.start)
...
def bw_3(bw, transfer_times, timestamps, current_time, lambda_decay=0.2):
...
def bw_4(bw, transfer_times, timestamps, current_time, lambda_decay=0.2, k=3):
    intervals = np.array([current_time - ts for ts in timestamps])
    bw_array = np.array(bw)
    transfer_times_array = np.array(transfer_times)
    ...
    if math.sqrt(observed_variance)/observed_mean > 0.5:
