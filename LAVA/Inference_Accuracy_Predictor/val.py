from utils import load_one_trace, V_Model
import pandas as pd
import numpy as np
import torch
import os
...
def calculate_MAE(df, pre_acc, n):
    true_acc = df.xs(n, level='CHUNK')['Accuracy'].tolist()
    mae_acc = sum(abs(pred - true) for pred, true in zip(pre_acc, true_acc)) / len(true_acc)
    return mae_acc

def test(net_env, acc_model, df, device):
    ...
    while True:
        ...
        if end_of_video != True:
            if theta_t >= t_l:
                ...
                llist, pre_acc, _ = fv_pre(V_model, acc_model, device, net_env.s, net_env.m, net_env.la,
