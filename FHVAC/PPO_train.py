        ...
        while True:
            for explore in range(exploration_size):
                ...
                for step in range(episode_steps):
                    ...
                    _, bw, latency, buffer_size, size, dynamics, f1, _, end_of_video = net_env.get_video_chunk(qp, skip, re)
                    reward = 2 * f1 - latency
                    ...
                    if end_of_video:
                        ...
                        net_env.video_start_shoot = net_env.start - 2  # 用来计算传输延迟
            ...
            logging.info('Epoch: ' + str(epoch) +
                         ...
                         # ' Avg_entropy_loss: ' + str((entropy1 + entropy2 + entropy3).detach().cpu().numpy()))

            if epoch % UPDATE_INTERVAL == 0:
                logging.info("Model saved in file")
                valid(model_actor, epoch, test_log_file, id)
